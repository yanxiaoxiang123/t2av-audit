...
6. Run `analyze_audio.py VIDEO --output-dir RUN/audio/primary` for a sound overview, then `audio_review.py RUN --output-dir RUN/audio/requirements` to check every audio claim in the frozen plan against full and local WAVs. Read `audio_requirements.json` before scoring. A general analysis omission is not absence. Use `confirmed_absent` only when full and covered local analyses explicitly agree; keep masked, conflicting and failed claims `无法判断`. The saved Gemini results are traceable candidates, not personal listening; keep `verified_by_listening=false` unless the current reviewer actually heard the audio. For uncertain claims, report every generated `listening_queue` clip. If a gateway request fails, rerun the same directory with `--resume`; successful batches are hash-checked and reused.
7. Build `RUN/draft.json` from the frozen plan, initial findings, challenge and audio requirement verdicts. Resolve visual disagreements from opened originals; unresolved audio conflicts remain `无法判断`. Numeric AV offsets require a confirmed sound, a common source timeline and sufficient timing precision; Gemini event times alone are estimates. Apply automatic uncertainty, severity, and metric-propagation caps before selecting rubric anchors.
