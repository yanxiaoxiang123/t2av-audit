...
6. Run `analyze_audio.py VIDEO --output-dir RUN/audio/primary` for a sound overview, then `audio_review.py RUN --output-dir RUN/audio/requirements` to check every audio claim in the frozen plan against full and local WAVs. Read `audio_requirements.json` before scoring. Keep `audibility_status` (sound/content heard), `fulfillment` (audio-side count or prohibition), `timing_status` (agreement between estimated onsets), and visual binding as separate judgments. A timing-only conflict must not erase confirmed presence or intelligible content; it remains a timing conflict and appears in prioritized `listening_clips`. Full/local disagreement about presence, event count, or a specific sound class remains `无法判断`. A general analysis omission is not absence. Use `confirmed_absent` only when full and covered local analyses explicitly agree. Keep masked and failed claims `无法判断`. The saved Gemini results are traceable candidates, not personal listening; keep `verified_by_listening=false` unless the current reviewer actually heard the audio. Start any manual listening from the deduplicated `listening_clips`; use `listening_queue` to trace each clip back to its requirements. If a gateway request fails, rerun the same directory with `--resume`; successful batches are hash-checked and reused.
7. Build `RUN/draft.json` from the frozen plan, initial findings, challenge and audio requirement verdicts. Resolve visual disagreements from opened originals. For a shot-bound sound or line, compare its saved source-time estimates with the visual cut PTS. If estimates disagree but still fall within the same shot, preserve the audible/content finding and record the coarse binding separately; if they straddle a cut or cannot be placed, keep the binding `无法判断`. For each resolved or contradictory binding, add `audio_binding` to the prompt check with its status, cited audio estimates, visual interval, reason and evidence IDs. Numeric AV offsets require a confirmed sound, a common source timeline and sufficient timing precision; Gemini event times alone are estimates, and a timing conflict requires an independent measurement before a numeric offset is allowed. Apply automatic uncertainty, severity, and metric-propagation caps before selecting rubric anchors.
